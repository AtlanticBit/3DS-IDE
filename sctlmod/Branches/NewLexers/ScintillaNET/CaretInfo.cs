...
namespace ScintillaNET
{
    ...
    public class CaretInfo : TopLevelHelper
    {
        ...
        /// </summary>
        /// <remarks>
        /// The <see cref="CurrentLineVisible"/> property must be set to true in order
        ...
        /// <summary>
        /// Gets or sets whether to highlight the current line even when the <see cref="Scintilla" /> control does not have focus.
        /// </summary>
        /// <returns>true to always highlight the current line; otherwise, false.</returns>
        [DefaultValue(false)]
        [Description("Indicates whether to highlight the current line, even when the control doesn't have focus.")]
        public bool CurrentLineAlwaysVisible
        {
            get
            {
                return (Scintilla.DirectMessage(NativeMethods.SCI_GETCARETLINEVISIBLEALWAYS, IntPtr.Zero, IntPtr.Zero) != IntPtr.Zero);
            }
            set
            {
                var alwaysVisible = (value ? new IntPtr(1) : IntPtr.Zero);
                Scintilla.DirectMessage(NativeMethods.SCI_SETCARETLINEVISIBLEALWAYS, alwaysVisible, IntPtr.Zero);
            }
        }
        ...
        /// </summary>
        /// <returns>true to highlight the current line; otherwise, false.</returns>
        /// <remarks>
        /// <see cref="CurrentLineBackgroundColor"/> determines the color.
        /// </remarks>
        [DefaultValue(false)]
        [Description("Indicates whether to highlight the current line.")]
        public bool CurrentLineVisible
        {
            get
            {
                return (Scintilla.DirectMessage(NativeMethods.SCI_GETCARETLINEVISIBLE, IntPtr.Zero, IntPtr.Zero) != IntPtr.Zero);
            }
            set
            {
                var visible = (value ? new IntPtr(1) : IntPtr.Zero);
                Scintilla.DirectMessage(NativeMethods.SCI_SETCARETLINEVISIBLE, visible, IntPtr.Zero);
            }
        }
