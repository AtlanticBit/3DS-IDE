    ...
    public class ConfigurationManager : TopLevelHelper
    {
        ...
        internal void Configure(List<Configuration> configList)
        {
            ...
            foreach (Configuration c in configList)
            {
                ...
                {
                    if (!resolvedIndicators.Contains(ic.Index) || !(ic.Inherit.HasValue && ic.Inherit.Value))
                    {
                        resolvedIndicators.Remove(ic.Index);
                        resolvedIndicators.Add(ic);
                    }
                    else
                    {
                        IndicatorConfig rc = resolvedIndicators[ic.Index];
                        ...
                        if (ic.DrawMode.HasValue)
                            rc.DrawMode = ic.DrawMode;
                    }
                }
            }
            ...
            {
                Indicator ind = Scintilla.Indicators[ic.Index];
                if (ic.Color != Color.Empty)
                    ind.Color = ic.Color;

                if (ic.DrawMode.HasValue)
                    ind.DrawMode = ic.DrawMode.Value;
			...
			//  Custom lexers do have a way to clear the properties,
			//  so we will call it.
			Scintilla.Lexing.ClearProperties();
            foreach (Configuration c in configList)
            {
            ...
            if (s == null)
            {
				if (Lexers.CustomLexer.IsCustomLexer(_language))
				{
					s = _language;
				}
                //	None of the configs specified a lexer. First let's see if
                //	we have a Language-Lexer map defined:
				else if (Scintilla.Lexing.LexerLanguageMap.ContainsKey(_language))
				{
			...
			foreach (Configuration c in configList)
			{
				foreach (KeyWordConfig kwc in c.Lexing_Keywords)
				{
					...
					{
						if (kwc.List.HasValue)
							Scintilla.Lexing.Keywords[kwc.List.Value] += kwc.Value;
						else
							Scintilla.Lexing.Keywords[kwc.Name] += kwc.Value;
					}
					else
					{
						if (kwc.List.HasValue)
