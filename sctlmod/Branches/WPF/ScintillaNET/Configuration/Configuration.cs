    ...
    public class Configuration
    {
        ...
        public void Load(XmlDocument configDocument)
        {
            ...
            if (caretNode != null)
            {
                ...
                if (blinkRate == null || blinkRate.ToLower() == "system")
            ...
            if (indicatorNode != null)
            {
                ...
                foreach (XmlElement el in indicatorNode.SelectNodes("Indicator"))
                {
                    ...
                    ic.Index = int.Parse(el.GetAttribute("Index"));
                    ic.Alpha = byte.Parse(el.GetAttribute("Alpha"));
                    ic.Color = getColor(el.GetAttribute("Color"));
                    ic.Inherit = getBool(el.GetAttribute("Inherit"));
                    ic.OutlineAlpha = byte.Parse(el.GetAttribute("OutlineAlpha"));
                    try
                    {
                        ic.DrawMode = (IndicatorDrawMode)Enum.Parse(typeof(IndicatorDrawMode), el.GetAttribute("DrawMode"), true);
                    }
                    catch (ArgumentException) { }
                    try
                    {
            ...
            if (lexerNode != null)
            {
                ...
                foreach (XmlElement el in lexerNode.SelectNodes("Keywords"))
                    _lexing_Keywords.Add(new KeyWordConfig(el.GetAttribute("Name"), getInt(el.GetAttribute("List")), el.InnerText.Trim(), getBool(el.GetAttribute("Inherit"))));
        ...
        private void ReadLexerKeywords(XmlReader reader)
        {
            ...
            _lexing_Keywords.Add(new KeyWordConfig(name, list, keywords, inherit));
