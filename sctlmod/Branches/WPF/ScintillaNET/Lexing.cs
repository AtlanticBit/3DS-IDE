...
namespace ScintillaNET
{
    ...
    public class Lexing : TopLevelHelper
    {
		...
		private readonly Dictionary<string, int> _keywordNameMap = new Dictionary<string, int>(StringComparer.OrdinalIgnoreCase);
		...
		private void loadKeywordMap()
		{
			...
			_keywordNameMap.Clear();

			string lexname = this.Lexer.ToString().ToLower();

			if ((lexname == "null" || lexname == "container") && !string.IsNullOrEmpty(CustomLexerName))
			{
				Lexers.CustomLexer.LoadKeywordNameMap(CustomLexerName, _keywordNameMap);
				return;
			}

			using (Stream s = GetType().Assembly.GetManifestResourceStream("ScintillaNET.Configuration.Builtin.LexerKeywordListNames." + lexname + ".txt"))
			{
				if (s == null)
					return;
				...
				using (StreamReader sr = new StreamReader(s))
				{
					...
					while (!sr.EndOfStream)
					{
						...
						if (str.Length == 0)
							continue;
						_keywordNameMap.Remove(str);
						_keywordNameMap.Add(str, keywordIndex);
						keywordIndex++;
					}
				}
			}
        ...
        private void loadStyleMap()
        {
			...
			if ((lexname == "null" || lexname == "container") && !string.IsNullOrEmpty(CustomLexerName))
			{
				Lexers.CustomLexer.LoadStyleNameMap(CustomLexerName, _styleNameMap);
				return;
			}
		...
		private readonly Dictionary<string, string> mAssignedProperties = new Dictionary<string, string>();
		internal Dictionary<string, string> AssignedProperties { get { return mAssignedProperties; } }
		/// <summary>
		/// This only has an effect on custom lexers,
		/// it does nothing for native lexers.
		/// </summary>
		internal void ClearProperties()
		{
			mAssignedProperties.Clear();
			if (curLexer != null)
				curLexer.ClearProperties();
		}
        public void SetProperty(string name, string value)
        {
			mAssignedProperties[name] = value;
		...
		[Browsable(false), DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
		public Dictionary<string, int> KeywordNameMap
		{
			get { return _keywordNameMap; }
		}
        ...
        public Lexer Lexer
        {
            ...
            set
			{
				if (value != Lexer.Container)
					CustomLexerName = null;
		...
		private string mCustomLexerName;
		public string CustomLexerName 
		{
			get { return mCustomLexerName; }
			set
			{
				curLexer = null;
				mCustomLexerName = value;
				if (!String.IsNullOrEmpty(value))
				{
					mCustomLexerName = value.ToLower();
					LexerName = "container";
					Lexer = Lexer.Container;
					loadStyleMap();
					loadKeywordMap();
		...
		private Lexers.CustomLexer curLexer;
		internal void Scintilla_StyleNeeded(object sender, StyleNeededEventArgs e)
		{
			if (curLexer != null && curLexer.LexerName == CustomLexerName)
			{
				curLexer.Style(e.Range.Start, e.Range.Text);
				return;
			}
			curLexer = null;
			Type t;
			if (CustomLexerName != null && Lexers.CustomLexer.TryGetCustomLexer(CustomLexerName, out t))
			{
				curLexer = (Lexers.CustomLexer)Activator.CreateInstance(t, Scintilla);
				curLexer.Style(e.Range.Start, e.Range.Text);
				return;
			}
		}
        ...
        public string LexerName
        {
            ...
            set
			{
				...
				if (string.IsNullOrEmpty(value))
					value = "null";
				if (Lexers.CustomLexer.IsCustomLexer(value))
				{
					this.CustomLexerName = value;
				}
				else
				{
