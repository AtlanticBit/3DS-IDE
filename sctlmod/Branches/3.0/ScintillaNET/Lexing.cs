...
namespace ScintillaNet
{
	...
	public class Lexing : TopLevelHelper
	{
		...
		public LexerType Lexer
		{
			get
			{
				return (LexerType)NativeScintilla.GetLexer();
			}
			set
			{
		...
		{
			return Lexer != LexerType.Container;
		}

		private void ResetLexer()
		{
			Lexer = LexerType.Container;
		}
		...
		private void loadStyleMap()
		{
			...
			_styleNameMap.Add("BRACEBAD", (int)Constants.STYLE_BRACEBAD);
			_styleNameMap.Add("BRACELIGHT", (int)Constants.STYLE_BRACELIGHT);
			_styleNameMap.Add("CALLTIP", (int)Constants.STYLE_CALLTIP);
			_styleNameMap.Add("CONTROLCHAR", (int)Constants.STYLE_CONTROLCHAR);
			_styleNameMap.Add("DEFAULT", (int)Constants.STYLE_DEFAULT);
			_styleNameMap.Add("LINENUMBER", (int)Constants.STYLE_LINENUMBER);
		...
		public void LineComment()
		{
			...
			PositionRange selRange = Scintilla.Selection.Range;
			...
			for (int i = start; i <= end; i++)
			{
				...
				if (firstWordChar >= 0)
				{
					Scintilla.InsertTextInternal(l.StartPosition + firstWordChar, _lineCommentPrefix);
					selRange.End += offset;
				}
			}
		...
		public void LineUncomment()
		{
			...
			for (int i = start; i <= end; i++)
			{
				...
				if (firstWordChar >= 0)
				{
					...
					PositionRange commentRange = Scintilla.GetRange(startPos, startPos + offset);
		...
		public void StreamComment()
		{
			...
			PositionRange selRange = Scintilla.Selection.Range;
			Scintilla.InsertTextInternal(selRange.Start, _streamCommentPrefix);
			Scintilla.InsertTextInternal(selRange.End+ _streamCommentPrefix.Length, _streamCommentSufix);
	...
	public class KeywordCollection : TopLevelHelper
	{
		...
		private Dictionary<string, LexerType> _lexerAliasMap;
			...
			: base(scintilla) 
		{
			...
			_lexerAliasMap = new Dictionary<string, LexerType>(StringComparer.OrdinalIgnoreCase);
			
			//	I have no idea how Progress fits into this. It's defined with the PS lexer const
			//	and a name of "progress"

			_lexerAliasMap.Add("PL/M", LexerType.Plm);
			_lexerAliasMap.Add("props", LexerType.Properties);
			_lexerAliasMap.Add("inno", LexerType.InnoSetup);
			_lexerAliasMap.Add("clarion", LexerType.Clw);
			_lexerAliasMap.Add("clarionnocase", LexerType.ClwNoCase	);
