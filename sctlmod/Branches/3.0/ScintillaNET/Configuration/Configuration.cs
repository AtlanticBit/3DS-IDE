...
namespace ScintillaNet.Configuration
{
	public class Configuration
	{
			...
			if (reader.HasAttributes)
			{
				...
				while (reader.MoveToNextAttribute())
					if (reader.Name.ToLower() == "bits")
						_enableUndo = getBool(reader.Value);
		...
		private void readUndoRedo(XmlReader reader)
		{
			...
			{
				while (reader.MoveToNextAttribute())
					if (reader.Name.ToLower() == "enableundo")
						_enableUndo = getBool(reader.Value);
		...
		private void readWrapping(XmlReader reader)
		{
			if (reader.HasAttributes)
			{
				while (reader.MoveToNextAttribute())
				{
					...
					{
						case "mode":
							_wrapping_Mode = (WrappingMode)Enum.Parse(typeof(WrappingMode), reader.Value, true);
							break;
						case "indent":
							_wrapping_Indent = getInt(reader.Value);
							break;
						case "indicator":
							_wrapping_Indicator = (WrappingIndicator)Enum.Parse(typeof(WrappingIndicator), reader.Value, true);
							break;
					}
				}
				reader.MoveToElement();
					...
					if (reader.NodeType == XmlNodeType.Element && reader.Name.Equals("binding", StringComparison.OrdinalIgnoreCase))
					{
						if (reader.HasAttributes)
						{
							...
							while (reader.MoveToNextAttribute())
							{
								...
								{
									case "key":
										kb.KeyCode = Util.GetKeys(reader.Value);
										break;
									case "modifier":
				...
				while (reader.MoveToNextAttribute())
				{
					...
					switch (attrName)
					{
						case "convertlinebreaksonpaste":
							_convertLineBreaksOnPaste = getBool(reader.Value);
							break;
					}
				}
				reader.MoveToElement();
		...
		public void Load(XmlDocument configDocument)
		{
			...
			{
				_convertLineBreaksOnPaste = getBool(clipboardNode.GetAttribute("ConvertLineBreaksOnPaste"));
			}
			clipboardNode = null;
			...
			if (commandsNode != null)
			{
				...
				{
					KeyBinding kb = new KeyBinding();
					kb.KeyCode = Util.GetKeys(el.GetAttribute("Key"));
			...
			XmlElement wrappingNode = langNode.SelectSingleNode("Wrapping") as XmlElement;
			if (wrappingNode != null)
			{
				try
				{
					_wrapping_Mode = (WrappingMode)Enum.Parse(typeof(WrappingMode), wrappingNode.GetAttribute("Mode"), true);
				}
				catch (ArgumentException) { }

				_wrapping_Indent = getInt(wrappingNode.GetAttribute("Indent"));

				try
				{
					_wrapping_Indicator = (WrappingIndicator)Enum.Parse(typeof(WrappingIndicator), wrappingNode.GetAttribute("Indicator"), true);
				}
				catch (ArgumentException) { }
			}
			wrappingNode = null;
			...
			{
				_scrolling_ScrollPastEnd = getBool(scrollingNode.GetAttribute("ScrollPastEnd"));
				_scrolling_HorizontalScrollTracking = getBool(scrollingNode.GetAttribute("HorizontalScrollTracking"));
				_scrolling_HorizontalScrollWidth = getInt(scrollingNode.GetAttribute("HorizontalScrollWidth"));
				...
				_scrolling_HorizontalScrollOffset = getInt(scrollingNode.GetAttribute("HorizontalScrollOffset"));
			}
			scrollingNode = null;
			...
			{
				_enableUndo = getBool(undoRedoNode.GetAttribute("EnableUndo"));
			}
			undoRedoNode = null;
		...
		{
			Color c = (Color)new ColorConverter().ConvertFromString(s);

			// For some reason alpha values don't get parsed right. If alpha
			// is zero and the color isn't empty we attempt to correct it.
			if (!c.IsEmpty && c.A == 0)
				c = Color.FromArgb(Byte.MaxValue, c);
		...
		#region Clipboard
		private bool? _convertLineBreaksOnPaste;
		public bool? ConvertLineBreaksOnPaste
		{
			get
			{
				return _convertLineBreaksOnPaste;
			}
			set
			{
				_convertLineBreaksOnPaste = value;
			}
		}
		#endregion
		...
		private int? _scrolling_HorizontalScrollOffset;
		public int? Scrolling_HorizontalScrollOffset
		{
			get
			{
				return _scrolling_HorizontalScrollOffset;
			}
			set
			{
		...
		private int? _scrolling_HorizontalScrollWidth;
		public int? Scrolling_HorizontalScrollWidth
		{
			get
			{
				return _scrolling_HorizontalScrollWidth;
			}
			set
			{
		...
		private bool? _scrolling_HorizontalScrollTracking;
		public bool? Scrolling_HorizontalScrollTracking
		{
			get
			{
				return _scrolling_HorizontalScrollTracking;
			}
			set
			{
				_scrolling_HorizontalScrollTracking = value;
			}
		}

		private bool? _scrolling_ScrollPastEnd;
		public bool? Scrolling_ScrollPastEnd
		{
			get
			{
				return _scrolling_ScrollPastEnd;
			}
			set
			{
		...
		#region Wrapping
		private WrappingMode? _wrapping_Mode;
		public WrappingMode? Wrapping_Mode
		{
			get
			{
				return _wrapping_Mode;
			}
			set
			{
				_wrapping_Mode = value;
			}
		}

		private WrappingIndicator? _wrapping_Indicator;
		public WrappingIndicator? Wrapping_Indicator
		{
			get
			{
				return _wrapping_Indicator;
			}
			set
			{
				_wrapping_Indicator = value;
			}
		}

		private int? _wrapping_Indent;
		public int? Wrapping_Indent
		{
			get
			{
				return _wrapping_Indent;
			}
			set
			{
				_wrapping_Indent = value;
			}
		}
		#endregion
	}
