...
{
	[Obsolete("Future builds may cripple functionality or remove the ConfigurationManager entirely. A replacement API will be available soon.")]
	...
	public class ConfigurationManager : TopLevelHelper
	{
		...
		internal void Configure(List<Configuration> configList)
		{
			...
			foreach (Configuration c in configList)
			{
				if (c.ConvertLineBreaksOnPaste.HasValue)
					b = c.ConvertLineBreaksOnPaste;
			}
			if (b.HasValue)
				Scintilla.ConvertLineBreaksOnPaste = b.Value;
			...
			if (s == null)
			{
				...
				else
				{
					try
					{
						Enum.Parse(typeof(LexerType), _language, true);
			...
			WrappingMode? wm = null;
			foreach (Configuration c in configList)
			{
				if (c.Wrapping_Mode.HasValue)
					wm = c.Wrapping_Mode;
			}
			if (wm.HasValue)
				Scintilla.Wrapping.Mode = wm.Value;
			
			i = null;
			foreach (Configuration c in configList)
			{
				if (c.Wrapping_Indent.HasValue)
					i = c.Wrapping_Indent;
			}
			if (i.HasValue)
				Scintilla.Wrapping.Indent = i.Value;

			WrappingIndicator? wvl = null;
			foreach (Configuration c in configList)
			{
				if (c.Wrapping_Indicator.HasValue)
					wvl = c.Wrapping_Indicator;
			}
			if (wvl.HasValue)
				Scintilla.Wrapping.Indicator = wvl.Value;
			...
			foreach (Configuration c in configList)
			{
				if (c.Scrolling_ScrollPastEnd.HasValue)
					b = c.Scrolling_ScrollPastEnd;
			}
			if (b.HasValue)
				Scintilla.Scrolling.ScrollPastEnd = b.Value;

			b = null;
			foreach (Configuration c in configList)
			{
				if (c.Scrolling_HorizontalScrollTracking.HasValue)
					b = c.Scrolling_HorizontalScrollTracking;
			}
			if (b.HasValue)
				Scintilla.Scrolling.HorizontalScrollTracking = b.Value;

			i = null;
			foreach (Configuration c in configList)
			{
				if (c.Scrolling_HorizontalScrollWidth.HasValue)
					i = c.Scrolling_HorizontalScrollWidth;
			}
			if (i.HasValue)
				Scintilla.Scrolling.HorizontalScrollWidth = i.Value;
			...
			foreach (Configuration c in configList)
			{
				if (c.Scrolling_HorizontalScrollOffset.HasValue)
					i = c.Scrolling_HorizontalScrollOffset;
			}
			if (i.HasValue)
			...
			foreach (Configuration c in configList)
			{
				if (c.EnableUndo.HasValue)
					b = c.EnableUndo;
			}
			if (b.HasValue)
				Scintilla.EnableUndo = b.Value;
